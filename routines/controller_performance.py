...
async def run(config: Config, context: ContextTypes.DEFAULT_TYPE) -> RoutineResult:
    ...
    try:
        ...
        # Use ts_all (snapshots + events) for volume charts so PMM controllers
        # (which rarely close executors) also appear. For non-PMM controllers,
        # ts_all already uses event-derived curves which are monotonic across deployments.
        try:
            fig_share = chart_volume_share(ts_all, config.bucket_hours)
        except Exception as e:
        ...
        try:
            fig_cumvol = chart_cumulative_volume(ts_all)
        except Exception as e:
